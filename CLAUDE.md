...
| `build-android.sh` | Builds core → `.so` + regenerates the Kotlin glue. Rerun after bumping the tag. Eight numbered `step_*` functions with a linear `main()` at the bottom — read that first. Its length is toolchain reconciliation, not build logic: cargo can't find the NDK linker, the host build must *not* see the NDK, and the NDK must match the one AGP strips with. The control surface is four env vars (`PROFILE`, `ABIS`, `ANDROID_API`, `BB_ORT`). |
...
| `README.md` / `BUILDING.md` | `README.md` is the **user-facing page** (pitch → privacy → open source), and is the source for store-listing copy — keep app claims in it true of the shipped app. `BUILDING.md` is the contributor half, split out of it 2026-08-22: build steps, flavours, `build-android.sh` knobs, the pre-PR flavour sweep, `scripts/`. This file stays the deeper *why*; when a build command changes, both move. |
