#!/usr/bin/env bash
#
# Manual: run the PRIVATE receipt corpus through the app's real scan pipeline on a
# connected Android device/emulator in LIVE mode, and diff against expected.json.
#
# Android twin of iOS scripts/sim-e2e-private.sh. This is the "device live mode"
# check for cases too PII-sensitive to publish. It is SLOW (real on-device OCR
# over the whole corpus) and needs private fixtures that don't exist in CI, so it
# never runs in CI — start it by hand and read the table.
#
...
# so nothing private is committed to this repo.
#
...
# surface.
#
#   scripts/android-e2e-private.sh [receipts_e2e-dir] [--all|--pilot]
#
# Paths default to the sibling checkout in this umbrella; override via env:
#   BB_PRIVATE_DIR=/path/to/beanbeaver-private-test  scripts/android-e2e-private.sh
#   PRIVATE_RULES=/path/to/private_rules.toml         (or empty to skip tolerance)
#
# Requires: an adb device in 'device' state and an arm64-v8a debug APK (or set
# BUILD=1 to have android-e2e.sh assemble+install first).
set -euo pipefail
...
echo "mode:          $MODE"
PRIVATE_RULES="$RULES" exec "$HERE/android-e2e.sh" "$FIXTURES" "$MODE"
